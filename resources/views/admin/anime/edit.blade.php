...
@section('title', 'Edit ' . $anime->title . ' - AniYume Admin')

@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-800">Edit Anime: {{ $anime->title }}</h1>
</div>
...
<form action="{{ route('admin.anime.update', $anime->id) }}" method="POST" class="bg-white rounded-lg shadow p-6">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="description" rows="5"
                      class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $anime->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="status" required
                    class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <option value="planned" {{ old('status', $anime->status) === 'planned' ? 'selected' : '' }}>Planned</option>
                <option value="ongoing" {{ old('status', $anime->status) === 'ongoing' ? 'selected' : '' }}>Ongoing</option>
                <option value="finished" {{ old('status', $anime->status) === 'finished' ? 'selected' : '' }}>Finished</option>
                <option value="paused" {{ old('status', $anime->status) === 'paused' ? 'selected' : '' }}>Paused</option>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="type" required
                ...
                <option value="special" {{ old('type', $anime->type) === 'special' ? 'selected' : '' }}>Special</option>
                <option value="music" {{ old('type', $anime->type) === 'music' ? 'selected' : '' }}>Music</option>
            </select>
        </div>
        ...
        <div class="md:col-span-2">
            <label class="flex items-center">
                <input type="checkbox" name="nsfw_flag" value="1" {{ old('nsfw_flag', $anime->nsfw_flag) ? 'checked' : '' }}
                       class="mr-2">
